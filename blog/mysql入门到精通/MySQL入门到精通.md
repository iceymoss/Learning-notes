...
* 修改表名：
  ...
  ```sql
  ALTER Table emp RENAME TO employee_info;
  ```
...
```sql
INSERT INTO user_info (id, name, age, gender) VALUES(1, 'iceymoss', 18, '男');

INSERT INTO user_info VALUES(2, 'kkdi', 19, '女');

INSERT INTO user_info (id, name, age, gender) VALUES(3, 'dmdk', 18, '男'),(4, 'alisi', 20, '女'),(5, 'kukudf', 21, '男'),(6, 'icldu', 19, '女');
```
...
```sql
UPDATE user_info SET name='lida' WHERE name='kkdi';

UPDATE user_info SET name='lili',gender='男' WHERE name='lida';

UPDATE user_info SET age=18;
```
...
```sql
DELETE FROM user_info WHERE gender='女';
...
```sql
select join_time as "入职时间" from emp_info;
```
...
select sum(id) from emp_info;

# 对所有女员工的年龄求和
select sum(age) from emp_info where gender = '女';
```
...
select work_adderss, count(*) from emp_info WHERE age < 45 group by work_adderss having count(*) > 3;
-- 起别名
...
# 根据在北京工作的员工的入职时间排序
select * from emp_info where work_adderss = "北京" order by join_time;
# 根据在北京工作的员工的年龄和入职时间排序
select * from emp_info where work_adderss = "北京" order by age, join_time;
```
...
```mysql
# 查询年龄为21,22,23的员工
select * from emp_info where age in(21,22,23);
# 查询年龄在20-40(包含)之间名字有三个字的女员工
select * from emp_info where gender = "女" && age >= 20 && age <= 30 && name like '___';
# 根据性别分类对小于60岁的员工进行数量统计
select gender,count(*) from emp_info where age < 60 group by gender;
# 查询年龄小于等于35的员工的姓名，年龄，入职时间并对其结果使用age升序排序，年龄相同时使用入职时间降序排序
select name, age, join_time from emp_info where age <= 35 order by age,join_time desc;
# 查询年龄在30-40(包含)的男员工中的前五名，并对结果根据年龄升序排序，年龄相同时根据入职时间升序排序
select * from emp_info where gender = "男" && age >= 30 && age <= 40 order by age asc, join_time asc limit 5;
```
